...
func (self *realPodsApi) List(nodeList *nodes.NodeList) ([]Pod, error) {
	...
	for _, pod := range pods {
		if _, ok := nodeList.Items[nodes.Node{pod.Status.Host, pod.Status.HostIP}]; ok {
...
func (self *realPodsApi) createAssignedPodLW() *cache.ListWatch {
...
func newPodsApi(client *client.Client) podsApi {
	...
	cache.NewReflector(podsApi.createAssignedPodLW(), &api.Pod{}, podsApi.podLister.Store).Run()
